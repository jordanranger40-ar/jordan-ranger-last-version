...
const Cart: React.FC<CartProps> = ({ data,action }) => {
  ...
  const total = useMemo(() => {
    return cartItems.reduce((sum, item) => sum + Number(item.total_amount || 0), 0) + shipping + tax;
  }, [cartItems]);
    ...
    <div className="lg:max-w-5xl max-lg:max-w-2xl mx-auto bg-white p-4 mt-20">
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-100 rounded-md p-6 md:sticky top-0 h-max">
          <h3 className="text-lg font-semibold text-[#484d23]">Order Details</h3>
          <hr className="border-gray-300 mt-4 mb-8" />
          ...
          <ul className="text-slate-600 font-medium mt-8 space-y-4">
            ...
            <li className="flex justify-between text-sm">
              Shipping <span className="text-[#484d23] font-semibold">${shipping.toFixed(2)}</span>
            </li>
            <li className="flex justify-between text-sm">
              Tax <span className="text-[#484d23] font-semibold">${tax.toFixed(2)}</span>
            </li>
