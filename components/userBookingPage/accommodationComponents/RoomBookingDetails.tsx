// components/userBookingPage/roomComponents/RoomBookingDetailsUser.tsx
"use client";
...
export default function RoomBookingDetailsUser({ data, locale }: Props) {
  const booking = data;
  ...
  const durationText = (() => {
    ...
    if (hours >= 24) {
      ...
      return rem ? `${days}d ${rem}h` : `${days}${isArabic?"أيام":"d"}`;
    }
    return `${hours}${isArabic ? "ساعة" : "h"}`;
  })();

  const unitPrice = Number(booking.price ?? 0);
  const total = Number(booking.booking_price ?? unitPrice);
    ...
    <div className="bg-white border rounded-2xl shadow p-4 sm:p-6" dir={isArabic ? "rtl" : "ltr"}>
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="w-full md:w-32 h-32 md:h-24 rounded-lg overflow-hidden bg-gray-100 shrink-0">
          <img
            src={booking.cover_image  ?? "/images/placeholder-room.jpg"}
            alt={isArabic ? booking.name_ar ?? "صورة الغرفة" : booking.name_en ?? "Room image"}
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-2">
            <div className="min-w-0">
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 wrap-break-word">
                {isArabic ? booking.name_ar ?? booking.name_en ?? "غرفة" : booking.name_en ?? "Room"}
              </h2>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-gray-700">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="font-medium">{formatDateTime(booking.start_time)}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="font-medium">{formatDateTime(booking.end_time)}</div>
              </div>
            </div>
