...
export default function ActivityBookingDetailsUser({ data, locale }: Props) {
  const booking = data;
  ...
  const unitPrice = Number(booking.activity_price ?? 0);
  const total = Number(booking.booking_price ?? unitPrice * (booking.quantity ?? 1));
    ...
    <div className="bg-white border rounded-2xl shadow p-4 sm:p-6" dir={isArabic ? "rtl" : "ltr"}>
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="w-full md:w-32 h-32 md:h-24 rounded-lg overflow-hidden bg-gray-100 shrink-0">
          <img
            src={booking.card_image ?? "/images/placeholder.jpg"}
            alt={isArabic ? booking.name_ar ?? "صورة النشاط" : booking.name_en ?? "Activity image"}
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-2">
            <div className="min-w-0">
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 wrap-break-word">
                {isArabic ? booking.name_ar ?? booking.name_en ?? "نشاط" : booking.name_en ?? "Activity"}
              </h2>
