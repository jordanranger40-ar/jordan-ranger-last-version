...
export default async function RoomPage({ params }: PageProps) {
  ...
  if (!data) {
    return <div className="text-center py-20 text-xl text-gray-700">Room not found</div>;
  }
  console.log(data);
  ...
  <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white max-w-3xl mx-auto px-6">
    ...
    <p className="text-lg md:text-xl text-gray-200">{data.description_en}</p>
  </div>
</div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 mt-12 md:mt-16 flex flex-col md:flex-row gap-12">
        <div className="flex-1 space-y-6">
          <h2 className="text-3xl md:text-4xl font-semibold text-[#333333]">{data.description_en}</h2>
          <p className="text-gray-600 leading-relaxed text-lg">{data.description_ar}</p>
          <div className="flex items-center gap-4">
            <span className="text-4xl font-extrabold text-[#676e32]">{data.price}</span>
            <span className="text-gray-500 text-lg">JOD / night</span>
          </div>
        ...
        <div className="flex-1">
          <h3 className="text-2xl font-semibold text-[#333333] mb-6">Room Amenities</h3>
            ...
            {data.features.map((feature: roomFeatures, index: number) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h4 className="font-semibold text-[#333333]">{feature.feature_title_en}</h4>
                </div>
                <p className="text-gray-600 text-sm leading-snug">{feature.feature_description_en}</p>
              </div>
            ))}
          </div>
        </div>
...
<div className="max-w-7xl mx-auto mt-16 px-6 md:px-12">
  <h3 className="text-2xl font-bold text-[#333333] mb-6">Gallery</h3>
