...
export const POST = async (request: Request) => {
  try {
    ...
    if (!authHeader) {
      ...
      const result = await bookAnActivity(body);
    ...
    return NextResponse.json(
      { data: error, message: " Error In Book The Activity" },
      { status: 500 }
    );
  }
};
  ...
  try {
    ...
    if (!authHeader) {
      ...
      if (payload.role !== "admin") {
        ...
        const result = await getAllActivitiesbookings();
        return NextResponse.json(
